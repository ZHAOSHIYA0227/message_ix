...
from message_ix.utiles import make_df
...
# demand must be met, and supply ca exceed demand allowing the model to plan for meeting demand in future periods by storing storable commodities. 
...
# The value in the input and output parameter is used to represent the efficiency of a technology (efficiency = output/input). For example, input of 1.0 and output of 1.0 for a technology shows that the efficiency of that technology is 100% in converting the input commodity to the output commodity.

# Extend "base_output" by filling in some of the other columns with keys, using the pandas.DataFrame.assign() method. 
bulb_out = base_input.assign(
    technology = "bulb",
    commodity = "light", 
    level = "useful",
    value = 1.0
...
#--------------------------------------------------#
#---- Parameterizaation of the electrical grid ----#
#--------------------------------------------------# 
...
for tec, val in lifetime.items():
    ...
    scenario.add_par("technical lifetime", df)
